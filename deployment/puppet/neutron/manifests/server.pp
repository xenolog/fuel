...
class neutron::server (
  ...
  neutron_config {
    ...
    'DEFAULT/nova_admin_tenant_id': value => 'XXX_service_tenant_id_XXX';
  ...
  exec {'insert service tenant ID':
    onlyif  => "bash -c \"source /root/openrc ; keystone tenant-list\" | grep 'services' | awk -F'|'  '{print \$2}' > /tmp/serviceid",
    command => "sed -e \"s/XXX_service_tenant_id_XXX/`cat /tmp/serviceid`/g\" -i /etc/neutron/neutron.conf",
    path => '/usr/sbin:/usr/bin:/sbin:/bin'
  } ->
  Service<| title == 'neutron-server' |>
  #Keystone_user_role <| title == "${neutron_config['keystone']['admin_user']}@services" |> -> Exec['insert service tenant ID']
