class osnailyfacter::cluster_simple {
  ...
  $sql_connection = "mysql+pymysql://nova:${nova_hash[db_password]}@${controller_node_address}/nova"
  ...
  case $::fuel_settings['role'] {
    ...
    "cinder" : {
      ...
      class { 'openstack::cinder':
        sql_connection       => "mysql+pymysql://cinder:${cinder_hash[db_password]}@${controller_node_address}/cinder?charset=utf8",
        glance_api_servers   => "${controller_node_address}:9292",
        queue_provider       => $::queue_provider,
        amqp_hosts           => $amqp_hosts,
        amqp_user            => $rabbit_hash['user'],
